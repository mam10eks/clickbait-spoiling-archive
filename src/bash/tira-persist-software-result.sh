...
TIRA_EVALUATION_OUTPUT_DIR="$(echo ${SRC_DIR}| awk -F '/output' '{print $1}')/../$(date +'%Y-%m-%d-%I-%M-%S')/output"
...
echo "TIRA_EVALUATION_COMMAND_TO_EXECUTE=echo '{\"key\":\"value\"}' >> \${TIRA_EVALUATION_OUTPUT_DIR}/evaluation-results.json" >> task.env
